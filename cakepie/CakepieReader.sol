...
contract CakepieReader is Initializable, OwnableUpgradeable {
    ...
    function getCakepiePoolInfo(
        ...
        } else if (poolInfo.poolType == V2Type || poolInfo.poolType == AMLType || poolInfo.poolType == StableSwapType) {
    ...
    function getMCakePooInfo(address account, address poolAddr) public view returns (CakepiePool memory)  {
    ...
    function getV2LikePoolInfo(
        ...
        cakepiePool.helper = ( V2LpLikepoolInfo.poolType == V2Type || V2LpLikepoolInfo.poolType == StableSwapType)
            ? pancakeV2LPHelper
            : pancakeAMLHelper;
    ...
    function getV2LikeAccountInfo(
        ...
        if (pool.poolAddress != mCake) {
            ...
            accountInfo.balance = ERC20(pool.depositToken).balanceOf(account);
            accountInfo.stakingAllowance = ERC20(pool.depositToken).allowance(account, address(masterCakepie));
    ...
    function getRewardInfo(
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            rewardInfo.pendingBonusDecimals[i] =  IERC20Metadata(rewardInfo.bonusTokenAddresses[i]).decimals();
        }
        return rewardInfo;
    }
    ...
    function getCakepieLockInfo(address account, address locker) public view returns (VlCakepieLockInfo memory) {
        ...
        if (account != address(0)) {
            try vlCakepieReader.getNextAvailableUnlockSlot(account) returns (uint256 nextAvailableUnlockSlot) {
                vlCakepieLockInfo.isFull = false;
            }
            catch {
                vlCakepieLockInfo.isFull = true;
            }
            vlCakepieLockInfo.userAmountInCoolDown = vlCakepieReader.getUserAmountInCoolDown(account);
            vlCakepieLockInfo.userTotalLocked = vlCakepieReader.getUserTotalLocked(account);
            IVLCakepieReader.UserUnlocking[] memory userUnlockingList = vlCakepieReader.getUserUnlockingSchedule(account);
            VlCakepieUserUnlocking[] memory vlCakepieUserUnlockingList = new VlCakepieUserUnlocking[](userUnlockingList.length);
            for(uint256 i = 0; i < userUnlockingList.length; i++) {
                ...
                if (locker == vlCKP) {
                    (uint256 penaltyAmount, uint256 amountToUser) = vlCakepieReader.expectedPenaltyAmountByAccount(account, i);
    ...
    function positionToPool(TokenIdPosition memory position) public view returns (address) {
        address pool = IPancakeV3Factory(v3Factory).getPool(position.token0, position.token1, position.fee);
        return pool;
    }

    /* ============ Internal Functions ============ */

    function _getAvailableTokenIdsInfo(address account) internal view returns (TokenIdInfo[] memory) {
        ...
        for (uint256 i = 0; i < totalAvailableToken; i++) {
            uint256 _tokenId = INonfungiblePositionManager(
    ...
    function _getStakedTokenIdsInfo(address account) internal view returns (TokenIdInfo[] memory) {
        ...
        TokenIdInfo[] memory availableTokenIdsInfo = new TokenIdInfo[](totalAvailableToken);
        for (uint256 i = 0; i < totalAvailableToken; i++) {
            uint256 _tokenId = IMasterChefV3(masterChefV3).tokenOfOwnerByIndex(
                account,
                i
            );
            TokenIdInfo memory idInfo;
    ...
    function _getCakepieStakedTokenIdsInfo(address account) internal view returns (TokenIdInfo[] memory) {
        ...
        for (uint256 i = 0; i < totalCKPStakedToken; i++) {
            ...
            ,
            ,
            idInfo.position.boostMultiplier) = masterChefV3.userPositionInfos(_tokenId);
    ...
    function _getTokenIdPositionInfo(uint256 tokenId)  internal view returns (TokenIdPosition memory) {
        TokenIdPosition memory position;
        (,,,,position.fee, position.tickLower,position.tickUpper,position.liquidity, position.feeGrowthInside0LastX128,position.feeGrowthInside1LastX128, position.tokensOwed0, position.tokensOwed1) = INonfungiblePositionManager(nonFungiblePositionManager)
            .positions(tokenId);
         (
            ,
            ,
